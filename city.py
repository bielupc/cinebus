...
def _build_carrer(carrer: dict) -> Carrer:
  ...
  speed = carrer.get("maxspeed", 30) # In case of no speed provided, it inputs the mode, which is 30
...
def build_city_graph(g1: OsmnxGraph, g2: BusesGraph) -> CityGraph:
  ...
  for stop in g2.nodes(data=True):
    ...
    if stop[0].municipality == "Barcelona":
      ...
      carrer = Carrer(uuid.uuid1(), False, f"{stop[0].stop_id}-{nearest_node}", "walk",WALKING_AVERAGE_SPEED, min_distance)
      G.add_edge(stop[0].stop_id, nearest_node,data=carrer, color="black", weight=min_distance/WALKING_AVERAGE_SPEED)

  # It adds all the edges from the OsmnGraph
  for carrer_data in g1.edges(data=True):
    carrer = _build_carrer(carrer_data[2])
    G.add_edge(carrer_data[0], carrer_data[1], data=carrer, color="blue", weight=carrer.length/carrer.maxspeed)
  ...
  for line_data in g2.edges(data=True):
    if line_data[0].municipality == "Barcelona" and line_data[1].municipality == "Barcelona":
      ...
      G.add_edge(line_data[0].stop_id, line_data[1].stop_id, data=_build_bus(line_data), color="red", weight=(dist/BUS_AVERAGE_SPEED) + BUS_LOAD_TIME)
...
def find_path(ox_g: OsmnxGraph, g: CityGraph, src: Coord, dst: Coord) -> Path:
  ...
  for node in route:
    if type(g.nodes(data=True)[node]["data"]) == Stop:
      print(stop)
  ...
  plot_path(g, path, "path.png")

  # plt.figure(figsize=(8, 6))

  # pos = nx.get_node_attributes(path, 'pos')
  # nx.draw(path, pos=pos, with_labels=False, node_size=50)
  # plt.show()
